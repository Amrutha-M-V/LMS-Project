...
//$start_date="SELECT * FROM leave_requests WHERE leave_requests='"$s_date"'; 
...
if($remaining >= 0 and $d_app <= $s_date and $s_date <= $e_date and $staff_leave != 'Invalid'){
  ...
  if(mysqli_query($db, $sql)){
   ...
   //if($s_date==$sunday){
    // header('location: apply.php?reason=invalid_leave'); 
     //echo"sunday holiday";
  //}
  if ($staff_leave == 'Invalid'){
    header('location: apply.php?reason=invalid'); 
  }
  else if ($s_date == $s_date)
   {
     header('location:apply.php?reason=invali');
   }
  ...
  {
    header('location: apply.php?reason=Sunday_holiday');
  }
